...
const temporaryRoot = await mkdtemp(path.join(temporaryParent, "development-e2e-"));
let playwright;
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") {
        reject(new Error("Could not allocate a local development E2E port."));
        return;
      }
...
function runPlaywright(environment) {
  return new Promise((resolve, reject) => {
    playwright = spawn("node_modules/.bin/playwright", ["test", ...process.argv.slice(2)], {
      cwd: process.cwd(),
      env: environment,
      stdio: "inherit"
    });
