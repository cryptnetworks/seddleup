...
| OAuth transient cookies                | `oauth_state_*` and `oauth_pkce_*`                                                           | Keep indefinitely               | Link purpose and target identity now live only in single-use server-side state. The former unsigned `oauth_link_*` cookie must not return. |
