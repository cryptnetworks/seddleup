  ...
  return (
    <main className="flex min-h-screen min-w-0 items-center justify-center bg-brand-page px-4 py-8 sm:py-10">
      <section className="auth-card w-full max-w-md p-4 sm:p-6">
        ...
        {query.verify ? (
          <p className="alert-success mt-4">Check your inbox for a verification link.</p>
        ) : null}
        {query.verified ? (
          <p className="alert-success mt-4">Email verified. Login to continue.</p>
        ) : null}
        ...
        {query.reset ? (
          <p className="alert-success mt-4">Password updated. Login with your new password.</p>
        ) : null}
        {query.logout ? <p className="alert-success mt-4">You have been logged out.</p> : null}
        {query.oauth && query.oauth !== "complete" ? (
          <p className="alert-error mt-4">OAuth sign-in failed: {query.oauth}.</p>
        ) : null}
        <div className="mt-6">
          <LoginForm />
        </div>
